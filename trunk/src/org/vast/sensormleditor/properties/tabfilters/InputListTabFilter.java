...
public class InputListTabFilter extends AbstractTabFilter {
	...
	public boolean select(Object toTest) {
		...
		if (toTest instanceof Element){
			...
			if (ele.getNodeName().equals("sml:inputs")) {
				stop = false;
				Element retNode = null;
				Element chosenOne = getChosenField(ele, retNode);
				if (chosenOne != null) {
					if (dom.hasQName(chosenOne, "sml:InputList"))
						return true;
				}
			}
		}
...
/*	@Override
	public boolean select(Object toTest) {
		if (toTest instanceof Element){
			Element ele = (Element) toTest;
			if (ele.getNodeName().equals("sml:InputList"))
				return true;
			
			else if (ele.getNodeName().equals("sml:input") && dataRecordSelected(ele))
				return true;
