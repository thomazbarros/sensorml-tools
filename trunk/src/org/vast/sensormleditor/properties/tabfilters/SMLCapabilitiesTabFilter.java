...
public class SMLCapabilitiesTabFilter extends AbstractTabFilter {
	...
	public boolean hasSMLCapabilities(Node node){
		NodeList children = node.getChildNodes();
		for (int i = 0; i < children.getLength(); i++){
			if (children.item(i).getNodeName().equals("sml:capabilities"))
				return true;
				...
				if (children.item(i).hasAttributes()){
					...
					for (int attIndex = 0; attIndex < attList.getLength(); attIndex++) {
						if (attList.item(attIndex).getNodeName().equals("name")){
							...
							if (value.equals("sml.capabilities"))
								return true;
						}
					}
				}
				...
				if (hasSMLCapabilities(children.item(i)))
					return true;	
		}
		return false;
	}
