...
public class SWEreferenceFramePropertySource extends AbstractPropertySource
		implements IPropertySource {

	public static final String PROPERTY_REFERENCE_FRAME = "SensorMLEditor.reference.frame";
	public PropertyDescriptor refFrameDescriptor;

	boolean stop = false;

	public SWEreferenceFramePropertySource(Element element,
		...
		for (int j = 0; j < children.getLength(); j++) {
			if (!children.item(j).getNamespaceURI().equals(rngaURI)) {
				if (dom.hasQName(children.item(j), "rng:ref")) {
					if (isSwereferenceFrame(children.item(j))) {
					...
					if (dom.getAttributeValue((Element) children.item(j),
							"name").equals("referenceFrame")) {
						refFrameDescriptor = new TextPropertyDescriptor(
								PROPERTY_REFERENCE_FRAME, "name");
						PropertyLabelProvider plp = new PropertyLabelProvider(
								node, dom);
						refFrameDescriptor.setLabelProvider(plp);
						stop = true;
	...
	public boolean isSwereferenceFrame(Node node) {
		String refName = dom.getAttributeValue((Element) node, "name");
		return refName.equals("swe.referenceFrame");
	}
	...
	public Object getPropertyValue(Object id) {
		String label = null;
		if ((id == PROPERTY_REFERENCE_FRAME) && (refFrameDescriptor != null)) {

			PropertyLabelProvider prop = (PropertyLabelProvider) refFrameDescriptor
					.getLabelProvider();
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if ((id == PROPERTY_REFERENCE_FRAME) && (refFrameDescriptor != null)) {
			PropertyLabelProvider propProvider = (PropertyLabelProvider) refFrameDescriptor
					.getLabelProvider();
