...
public class DateTimePropertySource extends AbstractPropertySource implements
		IPropertySource {
	public static final String PROPERTY_VALUE = "SensorMLEditor.gml.dateTime";
	public TextPropertyDescriptor nameDescriptor;

	public DateTimePropertySource(Element element, DOMHelper domHelper,
	...
	public void initProperties() {
		if (ele.getNodeName().equals("rng:data")&& dom.getAttributeValue(ele,"type").equals("dateTime")) {
			Node dateTime = ele;
			if (dateTime != null){
				nameDescriptor = new TextPropertyDescriptor(PROPERTY_VALUE,
				"type");

				PropertyLabelProvider plp = new PropertyLabelProvider(dateTime,
						dom);
	...
	public boolean isCountValue(Node node) {
		...
		return refName.equals("swe.booleanValue");
	}
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if ((id == PROPERTY_VALUE) && (nameDescriptor != null)) {
			...
			if (str != null && !str.equals("")) {
				if (!dom.existAttribute((Element) changeNode, "rng:value/@selected")) {
					dom.setAttributeValue((Element) changeNode,
							"rng:value/@selected", "true");
				}
				dom.setElementValue((Element) changeNode, "rng:value", value
						.toString());
