...
public class SMLClassifierPropertySource extends AbstractPropertySource
		implements IPropertySource {

	public static final String PROPERTY_SML_CLASSIFIER = "SensorMLEditor.sml.classifier";
	public TextPropertyDescriptor smlClassifierDescriptor;
	boolean stop = false;
	...
	public SMLClassifierPropertySource(Element element, DOMHelper domHelper,
	...
	public void recursiveInitProperties(Node node) {
		...
		for (int j = 0; j < children.getLength(); j++) {
			if (dom.hasQName(children.item(j), "rng:ref")) {
				if (isSMLClassifier(children.item(j))) {
				...
				handleoneOrMore(children.item(j));
					...
					.hasQName(node, "sml:classifier"))
					|| (dom.hasQName(children.item(j), "sml:classifier") && dom
							.existAttribute((Element) children.item(j), "name"))) {
				this.handleClassifiers(children.item(j));
	...
	public void handleoneOrMore(Node node) {
		...
		for (int j = 0; j < c.getLength(); j++) {
			...
			if (dom.hasQName(c.item(j), "a:documentation")) {
				continue;
			} else if (dom.hasQName(c.item(j), "sml:classifier")) {
				handleData(c.item(j));
			} else {
	...
	public void handleData(Node node) {
		...
		for (int j = 0; j < c.getLength(); j++) {
			if (dom.hasQName(c.item(j), "rng:attribute")) {
				...
				smlClassifierDescriptor = new TextPropertyDescriptor(
						PROPERTY_SML_CLASSIFIER, c.item(j).getLocalName());
				PropertyLabelProvider plp = new PropertyLabelProvider(ele, dom);
				smlClassifierDescriptor.setLabelProvider(plp);
			}
		}
	}

	public boolean isSMLClassifier(Node node) {
		String refName = dom.getAttributeValue((Element) node, "name");
		return refName.equals("sml.classifier");
	}

	public void handleClassifiers(Node node) {
		if (dom.hasQName(node, "sml:classifier")
	...
	public void handleSMLClassifier(Node node) {
		Node n = node;
		smlClassifierDescriptor = new TextPropertyDescriptor(
				PROPERTY_SML_CLASSIFIER, n.getNodeName());
		PropertyLabelProvider plp = new PropertyLabelProvider(
				n.getParentNode(), dom);
		smlClassifierDescriptor.setLabelProvider(plp);
	}
	...
	public Object getPropertyValue(Object id) {
		if ((id == PROPERTY_SML_CLASSIFIER)
				&& (smlClassifierDescriptor != null)) {
			return "";
		} else {
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if ((id == PROPERTY_SML_CLASSIFIER)
			...
			Node copyNode = dom.getElement((Element)oneOrMore,"sml:classifier");
