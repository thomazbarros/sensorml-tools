...
public class SMLFormatPropertySource extends AbstractPropertySource
		implements IPropertySource {
	public static final String PROPERTY_SML_FORMAT = "SensorMLEditor.sml.format";
	public TextPropertyDescriptor smlFormatDescriptor;
	boolean stop = false;

	public SMLFormatPropertySource(Element element, DOMHelper domHelper,
		...
		for (int j = 0; j < children.getLength(); j++) {
			if (!children.item(j).getNamespaceURI().equals(rngaURI)) {
				if (dom.hasQName(children.item(j), "rng:ref")) {
					if (isSMLFormat(children.item(j))) {
					...
					if (stop)
						return;
				} else if (dom.hasQName(children.item(j), "sml:format")) {
					handleSMLFormat(children.item(j));
					break;
				} else {
	...
	public boolean isSMLFormat(Node node) {
		String refName = dom.getAttributeValue((Element) node, "name");
		return refName.equals("sml.format");
	}

	public void handleSMLFormat(Node node) {
		Node n = node;
		smlFormatDescriptor = new TextPropertyDescriptor(
				PROPERTY_SML_FORMAT, n.getNodeName());
		PropertyLabelProvider plp = new PropertyLabelProvider(
				n.getParentNode(), dom);
		smlFormatDescriptor.setLabelProvider(plp);
		stop = true;
	}
	...
	public Object getPropertyValue(Object id) {
		String label = null;
		if ((id == PROPERTY_SML_FORMAT)
				&& (smlFormatDescriptor != null)) {
			String displayName = (String) smlFormatDescriptor
					.getDisplayName();
			label = smlFormatDescriptor.getLabelProvider().getText(
					displayName);
			return label;
		}
		return "";
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if ((id == PROPERTY_SML_FORMAT)
				&& (smlFormatDescriptor != null)) {
			PropertyLabelProvider propProvider = (PropertyLabelProvider) smlFormatDescriptor
					.getLabelProvider();
