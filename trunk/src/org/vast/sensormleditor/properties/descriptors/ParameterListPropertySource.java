...
public class ParameterListPropertySource extends AbstractPropertySource {
	...
	public ParameterListPropertySource(Element element, DOMHelper domHelper,
	...
	/*
	 * public void handleSWEField(Node node) { if (dom.hasQName(node,
	 * "swe:field")) { properties[propertyCount] = "SensorML.Property" +
	 * propertyCount; descriptor = new
	 * TextPropertyDescriptor(properties[propertyCount], "name");
	 * PropertyLabelProvider plp = new PropertyLabelProvider(node,dom);
	 * descriptor.setLabelProvider(plp); propertyDescriptors[propertyCount++] =
	 * descriptor; } }
	 */
	...
	public void handleoneOrMore(Node node) {
		...
		for (int j = 0; j < c.getLength(); j++) {
			...
			if (dom.hasQName(c.item(j), "a:documentation")) {
				continue;
			} else if (dom.hasQName(c.item(j), "sml:parameter")) {
				handleData(c.item(j));
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if (str != null && !str.equals("")) {
			if ((id == PROPERTY_SWE_FIELD) && (sweFieldDescriptor != null)) {
				...
				Element field = dom.getElement(ele,
						"rng:oneOrMore/sml:parameter");
