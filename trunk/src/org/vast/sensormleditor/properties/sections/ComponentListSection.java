...
import org.vast.sensormleditor.properties.descriptors.ComponentListPropertySource;
import org.vast.sensormleditor.properties.descriptors.DataRecordPropertySource;
...
public class ComponentListSection extends AbstractPropertySection {

	private String headerLabel = "ComponentList";
	...
	SMLTreeEditor smlEditor;
	private Element element;
	private DOMHelper dom;
	private TreeViewer treeViewer;
	private IPropertyDescriptor[] desc;
	private ComponentListPropertySource propertySource;
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		if (dom.hasQName(element, "sml:ComponentList")) {
			propertySource = new ComponentListPropertySource(element, dom,
			...
			if (newEle != null) {
				propertySource = new ComponentListPropertySource(newEle, dom,
						treeViewer, smlEditor);
				choicepropertySource = new ElementChoicePropertySource(newEle,
						dom, treeViewer, smlEditor);
			}
		}
...
/*	public Element getChosenField(Element element, boolean stop) {
		Element oneEle = null;
		NodeList children = dom.getChildElements(element);
		for (int i = 0; i < children.getLength(); i++) {
			oneEle = (Element) children.item(i);
			if (!dom.hasQName(oneEle, "rng:attribute")
				...
				} else if (dom.hasQName(oneEle, "rng:choice")) {
					if (!stop)
						oneEle = getChosenField(oneEle, stop);
				}
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (newfield != null && newfield != "") {
					...
					propertySource.setPropertyValue(
							DataRecordPropertySource.PROPERTY_SWE_FIELD,
							"Enter name...");
