...
public class SWEAxisIDPropertySource extends AbstractPropertySource
		implements IPropertySource {

	public static final String PROPERTY_AXIS_ID = "SensorMLEditor.axis.id";
	public PropertyDescriptor axisIDDescriptor;

	boolean stop = false;

	public SWEAxisIDPropertySource(Element element,
			...
			if (!children.item(j).getNamespaceURI().equals(rngaURI)) {
				if (dom.hasQName(children.item(j), "rng:ref")) {
					...
					if (dom.getAttributeValue((Element) children.item(j),
							"name").equals("axisID")) {
						axisIDDescriptor = new TextPropertyDescriptor(
								PROPERTY_AXIS_ID, "name");
						PropertyLabelProvider plp = new PropertyLabelProvider(
								node, dom);
						axisIDDescriptor.setLabelProvider(plp);
						stop = true;
	...
	public boolean isAxisID(Node node) {
		String refName = dom.getAttributeValue((Element) node, "name");
		return refName.equals("swe.axisID");
	}
	...
	public Object getPropertyValue(Object id) {
		String label = null;
		if ((id == PROPERTY_AXIS_ID) && (axisIDDescriptor != null)) {

			PropertyLabelProvider prop = (PropertyLabelProvider) axisIDDescriptor
					.getLabelProvider();
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if ((id == PROPERTY_AXIS_ID) && (axisIDDescriptor != null)) {
			PropertyLabelProvider propProvider = (PropertyLabelProvider) axisIDDescriptor
					.getLabelProvider();
