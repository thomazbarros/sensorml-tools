...
public class SWEDefinitionPropertySource extends AbstractPropertySource
	...
	public SWEDefinitionPropertySource(Element element,
		...
		for (int j = 0; j < children.getLength(); j++) {
			if (!children.item(j).getNamespaceURI().equals(rngaURI)) {
				if (dom.hasQName(children.item(j), "rng:ref")) {
					if (isSweDefinition(children.item(j))) {
	...
	public boolean isSweDefinition(Node node) {
		String refName = dom.getAttributeValue((Element) node, "name");
		return refName.equals("swe.definition");
	}
