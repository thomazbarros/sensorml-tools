...
public class OutputListPropertySource extends AbstractPropertySource {
	...
	public OutputListPropertySource(Element element, DOMHelper domHelper,
	...
	public void handleoneOrMore(Node node) {
		...
		for (int j = 0; j < c.getLength(); j++) {
			...
			if (dom.hasQName(c.item(j), "a:documentation")) {
				continue;
			} else if (dom.hasQName(c.item(j), "sml:output")) {
				handleData(c.item(j));
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if (str != null && !str.equals("")) {
			if ((id == PROPERTY_SWE_FIELD) && (sweFieldDescriptor != null)) {
				...
				Element field = dom.getElement(ele, "rng:oneOrMore/sml:output");
