...
public class DataRecordPropertySource extends AbstractPropertySource implements
		IPropertySource {
	...
	public DataRecordPropertySource(Element element, DOMHelper domHelper,
	...
	public void handleoneOrMore(Node node) {
		...
		for (int j = 0; j < c.getLength(); j++) {
			...
			} else if (dom.hasQName(c.item(j), "swe:field")) {
				handleData(c.item(j));
			}
		}
	}
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if (str != null && !str.equals("")) {
			if ((id == PROPERTY_SWE_FIELD) && (sweFieldDescriptor != null)) {
				...
				Node copyNode = dom.getElement((Element)oneOrMore,"swe:field");
