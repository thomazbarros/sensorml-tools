...
public class SWEFixedPropertySource extends AbstractPropertySource
		implements IPropertySource {

	public static final String PROPERTY_FIXED = "SensorMLEditor.fixed";
	public PropertyDescriptor fixedDescriptor;

	boolean stop = false;

	public SWEFixedPropertySource(Element element,
			...
			if (!children.item(j).getNamespaceURI().equals(rngaURI)) {
				if (dom.hasQName(children.item(j), "rng:ref")) {
					...
					if (dom.getAttributeValue((Element) children.item(j),
							"name").equals("fixed")) {
						fixedDescriptor = new TextPropertyDescriptor(
								PROPERTY_FIXED, "name");
						PropertyLabelProvider plp = new PropertyLabelProvider(
								node, dom);
						fixedDescriptor.setLabelProvider(plp);
						stop = true;
	...
	public boolean isSweFixed(Node node) {
		String refName = dom.getAttributeValue((Element) node, "name");
		return refName.equals("swe.fixed");
	}
	...
	public Object getPropertyValue(Object id) {
		String label = null;
		if ((id == PROPERTY_FIXED) && (fixedDescriptor != null)) {

			PropertyLabelProvider prop = (PropertyLabelProvider) fixedDescriptor
					.getLabelProvider();
	...
	public void setPropertyValue(Object id, Object value) {
		...
		if ((id == PROPERTY_FIXED) && (fixedDescriptor != null)) {
			PropertyLabelProvider propProvider = (PropertyLabelProvider) fixedDescriptor
					.getLabelProvider();
