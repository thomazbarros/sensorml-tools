...
public class AbstractDataRecordPropertySource extends AbstractPropertySource implements
		IPropertySource {

	public static final String PROPERTY_VALUE = "SensorMLEditor.abstract.datarecord";
	...
	public AbstractDataRecordPropertySource(Element element, DOMHelper domHelper,
	...
	public void recursiveInitProperties(Node node) {
		...
		for (int j = 0; j < children.getLength(); j++) {
			if (!children.item(j).getNamespaceURI().equals(rngaURI)) {
				...
				} else if (dom.hasQName(children.item(j), "swe:ConditionalData")
						|| dom.hasQName(children.item(j), "swe:ConditionalValue")
						|| dom.hasQName(children.item(j), "swe:DataRecord")) {
