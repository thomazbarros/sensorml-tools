...
import org.vast.sensormleditor.properties.descriptors.OutputListPropertySource;
...
public class OutputListSection extends AbstractPropertySection {

	private String headerLabel = "OutputList";
	...
	private OutputListPropertySource propertySource;
	...
	public void setInput(IWorkbenchPart part, ISelection selection) {
		...
		//We know that element should be either "sml:OutputList" or "sml:output"
		//If it is an output, we want to identify the type since it should be selected
		if (dom.hasQName(element, "sml:OutputList")){
			propertySource = new OutputListPropertySource(element, dom, treeViewer,smlEditor);
			choicepropertySource = new ElementChoicePropertySource(element, dom,
				treeViewer,smlEditor);
		} else {
			Element retNode = null;
			DOMHelperAddOn domUtil = new DOMHelperAddOn(dom);
			Element newEle = domUtil.getChosenField(element,retNode);
			if (newEle!=null){
				propertySource = new OutputListPropertySource(newEle,dom,treeViewer,smlEditor);
				choicepropertySource = new ElementChoicePropertySource(newEle, dom,
					treeViewer,smlEditor);
			}
		}

		String numType = propertySource.getTitle();
		headerlabelLabel.setText(headerLabel + ": " + numType);
	}
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (newfield != null && newfield != "") {
					...
					propertySource.setPropertyValue(
							OutputListPropertySource.PROPERTY_SWE_FIELD,
							"Enter name...");
			...
			public void widgetSelected(SelectionEvent event) {
				...
				for (int i = 0; i < desc.length; i++) {
					...
					if (item.equals(remfield)) {
						choicepropertySource.removeField(desc[i].getId(), remfield);
					}
				}
